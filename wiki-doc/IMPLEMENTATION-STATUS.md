# Актуальная проверка и исправления — 2026-09-13

Проверены и исправлены прежние замечания P0-02/P0-03 и новые реализации
P0-04, P0-05 и P1-01. Подробности и границы приёмки:
[REVIEW-P0-P1-VERIFICATION.md](wiki-doc/REVIEW-P0-P1-VERIFICATION.md).
Эта запись заменяет промежуточные статусы и числа тестов в истории ниже.

| Пункт | Актуальный результат |
|---|---|
| P0-02 | Проверен и исправлен: каталог, применимость, критичность, отдельные обязательства выражений и точные якоря |
| P0-03 | Проверен и исправлен в поднаборе P0: независимый SQL-инвентарь, точные границы, вызовы, зависимости, выражения; неполный разбор блокируется |
| P0-04 | Проверен и исправлен: реальные доказательства, хэши прочитанных байтов, снимок реестра идентичности, архив проверенного комплекта |
| P0-05 | Проверен и исправлен: повторное построение inventory/plan, сверка фактов и идентичности, отказ при подменах и технических дефектах, положительный ready |
| P1-01 | Проверен и исправлен: нормализация PostgreSQL, режимы параметров, quoted identifiers, разделители, старые пути/ключи и коллизии; интегрирован с gate |

**Итоговый прогон:** 382 теста, **381 прошёл, 1 пропущен** — создание symlink
недоступно в текущем Windows-окружении. Добавлена 41 регрессия.
Проверены все 11 SQL-примеров через CLI (16 срезов), схемы и связи inventory/plan,
quick_validate, ссылки изменённых инструкций и git diff --check.

Положительные комплекты routine, view и dynamic SQL с честным unknown проходят gate;
искажённые выражения, пропущенная колонка, подменённый template, ключ или произвольный
page_id отклоняются. Сохранение прежнего пути требует явного хэшируемого реестра.

**Далее по плану:** P1-02/P1-03 и последующие пункты. Полный разбор SQL, Markdown-
фрагментов и приёмка генерации агентом не объявлены завершёнными. Подробная матрица
ограничений P0 приведена в отчёте и references/artifacts.md. Содержательная сверка
SQL → факты → текст остаётся обязательной.
...
P1-02: полноценная проверка покрытия текста (`scripts/coverage_gate.py`).
