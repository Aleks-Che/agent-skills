...
**Итог повторной проверки:** 662 passed, 1 skipped (Windows symlink), 0 failed;
...
- `tests/test_dialect_matrix.py`: 28 тестов матрицы диалектов.
- Полный прогон на 2026-09-19 (включая test_publish и test_p1_acceptance):
  **652 passed, 1 skipped, 0 failed; 182 subtests passed**. Пропуск — создание
  symlink, недоступное на этой Windows; известного subprocess-отказа больше нет.
...
P2-05: эксплуатация — общая CLI-обёртка, версия пакета, CHANGELOG, журнал этапов
и документация одиночного/конкурентного режима. Пункт не начат.
