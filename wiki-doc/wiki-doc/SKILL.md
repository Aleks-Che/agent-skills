...
2. Определи диалект, версию СУБД и документируемую ревизию из проекта. Если версия
   ...
   См. [матрицу поддержки диалектов](docs/dialect-support.md) для списка подтверждённых
   конструкций и ограничений.
...
Исполни `python scripts/metrics.py <wiki_dir> [--json] [--regression-report <path>]`
для сводных метрик качества документации (REVIEW §8.2):

- `fresh_sources` — доля страниц с актуальными хэшами источников;
- `full_coverage` — доля страниц с полным покрытием фактов;
- `first_pass_success` — доля страниц, прошедших gate с первого раза;
- `blocking_defects` — суммарное блокирующих дефектов;
- `open_unknowns` — суммарное открытых неопределённостей;
- `outdated_pages` — страницы с устаревшими источниками;
- `broken_links` — битые относительные ссылки;
- `erroneous_ready` — доля ошибочных `ready` на контрольном наборе
  (`numerator`/`denominator`/`percent`, цель: 0);
- `average_iterations` — среднее число итераций исправления в текущем run страницы.

Метрика безопасности: `erroneous_ready.percent` на размеченном мутационном наборе
должна быть нулевой. Без `mutation-report.json` от `scripts/regression_mutations.py`
(`--regression-report`) прогон
считается непройденным: `measured` = false, `percent` = null, а не успешный 0.
Периоды, знаменатели, история gate и обработка пропусков описаны в
[references/metrics.md](references/metrics.md). Для старых публикаций без истории
`first_pass_success` и `average_iterations` остаются неизмеренными.
