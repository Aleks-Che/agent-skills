...
`DISTRIBUTED BY`, свойства хранения и `EXECUTE ON MASTER` не разбираются парсером PostgreSQL (libpg_query): при наличии в SQL они дают блокирующий `coverage_note` о сбое анализа, а не подтверждение поддержки. Профиль фиксирует наличие MERGE в PostgreSQL начиная с 15, но отдельно запрещает переносить этот вывод на неизвестную версию Greenplum.
