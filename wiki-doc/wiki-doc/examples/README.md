...
заявляет совместимость с неизвестной версией Greenplum.
...
находятся в `expected/01`…`expected/11`; они доступны только проверяющей стороне.
...
`validation_gate.py` пока вычисляет результат только по отчёту: код 0 — ready,
1 — revise/blocked, 2 — некорректный вход. Этот результат не подтверждает независимую
полноту, действительность доказательств или готовность комплекта к публикации.
Полный gate относится к P0-05. Приведённые проверки не исполняют SQL и не заменяют
описанный выше содержательный прогон 11 сценариев.
