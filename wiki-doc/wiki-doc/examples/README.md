...
Набор проверяет факты и решения агента. PostgreSQL-примеры 01–10 используют синтаксис
PostgreSQL 15+; пример 11 дополнительно проверяет проектные правила CKR_GP, но не
заявляет совместимость с неизвестной версией Greenplum. Пример 12 проверяет
расширения модели: триггеры, индексы, ограничения и GRANT/REVOKE.
...
находятся в `expected/01`…`expected/12`; они доступны только проверяющей стороне.
...
| [12](12_triggers_indexes.sql) / [ожидания](expected/12/facts.json) | Не требуется | Триггеры, индексы, ограничения и GRANT/REVOKE от SQL до gate |

Пример 12 входит в общий manifest. В `expected/12` заданы независимые ожидания
и восемь искажений, каждое проверяется в режимах text-only и coherent.
`tests/test_p2_03_acceptance.py` также проверяет согласованное удаление операций
и ограничений из фактов/страницы при пересобранном manifest.
...
Вызов `validation_gate.py` с одним отчётом даёт только диагностику и не разрешает
публикацию. Полный gate вызывается с `--bundle <run_dir> --write-decision` и
корнями проекта/wiki; код 0 означает ready проверенного комплекта, 1 — отказ,
2 — ошибку входа. Проверки не исполняют SQL и не заменяют содержательный просмотр
документации по всем 12 сценариям.
