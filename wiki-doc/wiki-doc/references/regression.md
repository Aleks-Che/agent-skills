...
[cases.json](../examples/cases.json) задаёт 13 случаев и 17 отдельных предметов страниц.
...
Мутация с полем `subject` применяется только к указанной странице; без этого поля —
ко всем предметам случая. Это позволяет отдельно проверять таблицу с триггером и
журнал аудита в случае 13, не приписывая им операции друг друга.
...
Копируются VERSION и CHANGELOG, а из `docs/` — `dialect-support.md`,
`operations.md` и `agent-response-templates.md`, на которые ссылается SKILL.md.
Ссылки матрицы на тесты и общий каталог примеров служат доказательствами в исходном
пакете; эти материалы не передаются изолированному генератору.
