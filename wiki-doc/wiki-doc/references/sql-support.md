...
от его каталога внутри project root. Диалекты `postgres`/`postgresql` принимаются
без учёта регистра, как в SQL-инвентаре. mtime не используется. Нет установленного порядка —
...
Матрица проверенного поднабора диалектов, версионные ограничения и непроверенные области:
[docs/dialect-support.md](../docs/dialect-support.md).
